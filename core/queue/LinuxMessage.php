...
class LinuxMessage implements QueueInterface
{
    /**
     * @var int channel
     */
     ...
     * @throws \Exception
     */
    protected function initQueue($ipc_filename, $channel)
    {
        $this->key_t = $this->getIpcKey($ipc_filename, $channel);
        $this->queue = \msg_get_queue($this->key_t);
        if (!$this->queue) {
            trigger_error('msg_get_queue failed');
        };
    }
    ...
    public function getIpcKey($ipc_filename, $msg_type)
    {
        if (!file_exists($ipc_filename)) {
            ...
            if ($create_file === false) {
                trigger_error('ipc_file is not exists and create failed');
            }
        }

        $key_t = \ftok($ipc_filename, $msg_type);
        if ($key_t == 0) trigger_error('ftok error');
